...
| 来源 | 负责 | 说明 |
|---|---|---|
| Binance 公开 API | 行情/资金费率/OI/多空比/主动买卖/深度 | 价格的**唯一权威源** |
| HTX 公开 API | 资金费率/OI/深度/行情 | 用户所在场所；跨场所资金费率差是独立信号 |
| **dogdoing.ai** 公开 JSON 接口 | 社交热度、OI 背离、资讯、情绪、链上代币安全 | `/api/square-hype` `/api/oi-divergence` `/api/fear-greed` `/api/news` `/api/market-tickers` `/api/token-audit` `/api/token-info`；**价格仅作交叉校验** |
| CoinGecko | 市值/FDV/流通量/全市场/BTC占比/稳定币代理 | 免费额度 |
| alternative.me | 恐惧贪婪指数 | 与 dogdoing 双源比对 |
...
- **dogdoing 是内部 JSON 接口**（非官方 API，无文档），若某天 404/改结构，用 `--no-dogdoing` 降级并在报告标注"社交热度未取到"；不要编造热度数据。
- **BTC 自身不做相对 BTC 强弱**（脚本已特判标注）。
- **免费接口拿不到的**：清算明细、链上交易所余额/巨鲸转账、精确解锁日历、合规/审计现状 → 一律标注"未核验"，不得推测。
- **CoinGecko 免费额度会耗尽**（10,000 次/日）：`/global` 与 `/coins/markets` 会间歇性失败 → 脚本显示"—"并写入缺口清单；此时用 `--no-cg` 跑，或等额度恢复。**不要把上次取到的旧值当本次数据用。**
