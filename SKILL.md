...
7. **执行层（真实下单，默认 dry-run）**：
   ...
   - **两档巡检（频率与噪声的权衡）**：
     | 档位 | 频率 | 命令 | 作用 |
     |---|---|---|---|
     | 持仓管理（静默） | **每 15 分钟** | `autotrade.py --manage-only`（cron no_agent，无动作则空输出=不打扰） | 复核持仓论点，反转/消失→平仓，弱化→收紧止损 |
     | 建仓扫描 | **每 60 分钟** | `autotrade.py --scan 8 --include-tradfi` | 扫描榜单新机会并建仓 |

     **⚠️ 频率提高 = 噪声抽样次数同比增加，必须同时配 anti-churn 护栏（否则把亏损机制放大数倍）**：
     | 护栏 | 取值 | 防的是什么 |
     |---|---|---|
     | 最小持仓时间 | 30 分钟 | 15 分钟巡检会 4 倍化"假极端读数"；模型驱动的退出不得早于 30 分钟 |
     | 同标的冷却 | 平仓后 6 小时 | 防"开→被噪声平→再开"的 churn 循环（实测置信度≥0.65 的读数仅占约 6%，60 分钟扫描意味着约 11 个开仓信号/日，而仓位上限只有 2） |
     | ~~每日开仓额度~~ | **已取消**（`MAX_ENTRIES_PER_DAY = None`，用户指定不限制） | 保留函数，填数字即恢复 |
     | 极端平仓需两次确认 | 15 分钟间隔 | 单次噪声不再立即平仓（原"极端读数立即平仓"在 15 分钟频率下误触发率翻倍） |
   - **为什么管理要高频**：止损离入场常仅 1% 左右，论点置信度可能在半小时内从 0.60 掉到 0.17；4 小时等于让"论点已死"的仓位多暴露数小时。**为什么扫描不必更勤**：4H 是主决策周期（结构位只在 4H 收盘变化），且建仓有并发上限 2 + 严格门控，扫得更勤只会增加噪声决策。
   - **频率提高必须配确认机制**（否则被模型噪声甩单）：平仓分两档 —— `极端读数`（方向概率<0.30 或 置信度<0.20）立即处置；`普通读数`需**连续两次**管理读数一致才平仓，首次仅记 `CLOSE_PENDING`。原因：TypeSafe 对同一状态复跑会出现不同答案（实测同状态置信度 0.45 vs 0.10）
   - **交易所侧 algo 条件单 24/7 独立运行，不依赖巡检间隔** —— 这是突发反向的第一道防线，巡检频率再低也不会失去止损保护
...
### 加密 / tradfi 实盘坑位（2026-09-22 首日实盘踩出来的）

- **`contractType` 不止 PERPETUAL**：Binance tradfi 合约是 **`TRADIFI_PERPETUAL`**。只筛 `=="PERPETUAL"` 会**静默漏掉全部 199 个**股票/ETF/商品合约（现象：App 里明明有 TQQQ/SPY/XAU，API 枚举却查不到）。凡枚举一律 `contractType in ("PERPETUAL","TRADIFI_PERPETUAL")`。
- **tradfi 的相对强弱不能用 BTC 做基准**：黄金/股票与加密无共同驱动 → 已改用 `SPYUSDT`，字段为 `vs_spy` / `corr_beta_spy_90d`。
- **tradfi 无现货 K 线**：`/api/v3/klines` 对 tradfi 返回 400，必须走 `/fapi/v1/klines`（脚本已自动回退）。
- **tradfi 止损必须走 Algo Order API**：标准端点不支持 STOP_MARKET/TAKE_PROFIT_MARKET，会返回 `-1102 Order type not supported`。正确调用：`POST /fapi/v1/algoOrder`，参数 **`algoType=CONDITIONAL` + `triggerPrice`**（不是 stopPrice）；查询 `/fapi/v1/openAlgoOrders`，撤销 `DELETE /fapi/v1/algoOrder`（algoId/clientAlgoId）。**下单后必须校验止损单是否挂上，未挂上立即平仓**（否则仓位裸奔）。
- **tradfi 有明显时段效应**（实测近 30 天 1H K线）：美股现金时段只占全天 27% 时间却占成交额的 **29.5%–40.7%**（NVDA 40.7%、MU 35.2%、SOXL 32.9%、XAU 29.5%）→ 每小时强度是休市时段的 **2.0–3.2 倍**；**周末仅占 4.0%–5.7%**（每小时约为工作日 1/6）。对照加密：BTC 现金时段占比 29.8% ≈ 时间占比 27%，**无时段效应**。→ 故 `session_gate()`：tradfi **周末/美股假期禁止新开仓**；**深夜 00:00–09:00 UTC 要求止损 ≥1.5% 且价差 ≤5bps**；盘前/盘后**减半仓**；现金时段正常；加密不受限。
- **仓位必须从「实际挂出的止损」反推**（曾经的严重不一致）：先前用 2×ATR 估算止损算仓位，但实际挂单止损可能只有 0.9% → 实盘那笔名义 $13 只承担了 **$0.119** 风险（预算的 24%）；反之若实际止损更宽则会**超预算**。正确做法：先定 plan(entry/stop/tp)，再用 `风险预算 ÷ 实际止损距离 = 名义`，并校验最小名义。**永远不要用两套止损口径**。
- **成本才是主要磨损**：首两笔实盘合计净亏 −0.0747 USDT，其中**手续费占 59%**（价格亏损仅 −0.0308）。含成本盈亏平衡胜率 `p* = (r+c)/(r(RR+1))`：RR=2.5、r=1%、c≈7.5bps → **30.7%**（理论线仅 28.6%）。评估必须用含成本口径。
- **退出不要只看"置信度"**：置信度噪声极大（同状态实测 0.45 与 0.10）。出现过"方向概率 0.58（仍同向、高于入场门槛 0.55）但置信度 0.37 → 平仓"→ 等于**用比开仓更弱的理由平仓**，白付两次手续费（churn）。退出**只由方向概率退化驱动**：<0.52 连续两次、或 <0.40、或反转且 conf≥0.65（立即）。
